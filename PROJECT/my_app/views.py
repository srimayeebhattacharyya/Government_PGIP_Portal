...
def dashboard(request):
    ...
    e_eligibilities = request.GET.getlist('e_eligibility')  # from checkbox values
    ...
    if e_eligibilities:
        exams = exams.filter(e_eligibility__in=e_eligibilities)  # ✅ correct field

    if dates:
        exams = exams.filter(date__in=dates)
...
def exam_detail(request, exam_id):
    ...
    if request.user.is_authenticated:
        ...
        if not Task.objects.filter(
            ...
            created = create_calendar_reminder(request.user, f"Exam: {exam.name}", exam.date)
            if created:
                messages.info(request, f"Added {exam.name} to your calendar!")
        else:
            messages.warning(request, f"{exam.name} is already in your calendar!")
...
def scheme_detail(request, scheme_id):
    ...
    if request.user.is_authenticated:
        ...
        if not Task.objects.filter(
            ...
            created = create_calendar_reminder(request.user, f"Scheme: {scheme.name}", scheme.date)
            if created:
                messages.info(request, f"Added {scheme.name} to your calendar!")
        else:
            messages.warning(request, f"{scheme.name} is already in your calendar!")
...
def details_view(request, item_type, item_id):
    context = {
        "item_type": item_type,
        "item_id": item_id,
        "name": f"Dummy {item_type.title()} {item_id}",
        "description": "This is a placeholder description. Later you can replace it with real exam/scheme link."
    }
    return render(request, "details.html", context)